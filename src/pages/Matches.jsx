...
const Matches = () => {
  ...
  const [oppositeProfiles, setOppositeProfiles] = useState([]);

  useEffect(() => {
    getOppositeGenderProfiles();
  }, [filters]); 

  const getOppositeGenderProfiles = async () => {
    try {
      const res = await getData("user/auth/users-opposite-gender", null);
    
      setOppositeProfiles(res?.users || []);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center">
          ...
          Matches ({(oppositeProfiles.length || filteredProfiles.length)})
        </h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {(oppositeProfiles.length ? oppositeProfiles : filteredProfiles).map(
          (profile) => (
            <ProfileCard
              key={profile.id}
              profile={profile}
